...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch news, analyze sentiment, and generate a music prompt.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a local news JSON file to use instead of fetching."
    )
    ...
    if args.analyze:
        if all_regional_data:
            ...
            for region_data in all_regional_data.values():
                ...
                if articles and isinstance(articles, list): # ** Check
                    all_articles.extend(articles)

            if not all_articles:
                print("No articles available to analyze.")
            else:
                music_prompt, sentiment_analysis = llm_analyzer.generate_music_prompt_from_news(all_articles)
            ...
            print("\nAnalysis enabled, but no data could be loaded. Please run with --fetch True or provide a --local-file.")